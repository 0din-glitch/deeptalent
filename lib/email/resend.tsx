...
export async function sendVerificationCodeEmail(
  ...
  try {
    ...
    const digitCells = code
      ...
      .map(
        (d) =>
          `<td style="padding:0 5px;"><div style="width:52px;height:64px;line-height:64px;text-align:center;font-family:'SF Mono','Segoe UI Mono','Roboto Mono',Menlo,Consolas,monospace;font-size:30px;font-weight:700;color:#1f2937;background:#ffffff;border:2px solid #e5e7eb;border-radius:12px;box-shadow:0 2px 6px rgba(15,23,42,0.06);">${d}</div></td>`
      )
      .join("");

    const firstName = (fullName || "there").split(" ")[0];

    const html = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1.0"/><title>Your DeepTalent verification code</title></head><body style="margin:0;padding:0;background-color:#eef1f8;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;color:#374151;">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#eef1f8;padding:32px 0;">
        <tr><td align="center">
          <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:18px;overflow:hidden;box-shadow:0 10px 30px rgba(36,49,110,0.12);">
            <!-- Header -->
            <tr><td style="background:linear-gradient(135deg,#24316e 0%,#3B5BDB 55%,#8690FD 100%);padding:44px 32px;text-align:center;color:#ffffff;">
              <p style="margin:0;font-size:13px;letter-spacing:2px;text-transform:uppercase;opacity:.85;">DeepTalent</p>
              <h1 style="margin:12px 0 0 0;font-size:26px;font-weight:700;letter-spacing:-0.4px;">Verify your email</h1>
              <p style="margin:8px 0 0 0;font-size:14px;opacity:.9;">One quick step to secure your account.</p>
            </td></tr>
            <!-- Body -->
            <tr><td style="padding:40px 40px 8px 40px;">
              ...
              <p style="margin:0 0 28px 0;font-size:15px;line-height:1.7;color:#4b5563;">Use the verification code below to confirm your email address and finish setting up your DeepTalent account.</p>
              <!-- Code -->
              ...
              <div style="background:#eef2ff;border-left:4px solid #3B5BDB;border-radius:8px;padding:14px 18px;margin:28px 0 0 0;font-size:13px;color:#1e3a8a;line-height:1.7;">
            ...
            <tr><td style="padding:28px 40px 40px 40px;">
              <div style="border-top:1px solid #e5e7eb;padding-top:22px;text-align:center;font-size:13px;color:#9ca3af;">
                <p style="margin:0 0 6px 0;font-weight:600;color:#6b7280;">DeepTalent</p>
    ...
    const result = await resend.emails.send({
      ...
      to: email,
      replyTo: REPLY_TO_EMAIL,
      subject: `${code} is your DeepTalent verification code`,
      html: withReplyNotice(html),
    });

    if (result.error) {
      console.error("[Resend] Verification code API error:", result.error);
      return { success: false, error: result.error.message || JSON.stringify(result.error) };
    }

    console.log("[Resend] Verification code sent:", { to: email, id: result.data?.id });
    return { success: true, messageId: result.data?.id || "sent" };
  } catch (error: any) {
    console.error("[Resend] Verification code exception:", error?.message || error);
    return { success: false, error: error?.message || String(error) };
  }
}
...
export async function sendNyscVerificationCodeEmail(
  ...
  try {
    ...
    const html = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1.0"/><title>Your DeepTalent Post-NYSC verification code</title></head><body style="margin:0;padding:0;background-color:#eefbf3;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;color:#374151;">
        ...
        <tr><td align="center">
          <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:18px;overflow:hidden;box-shadow:0 10px 30px rgba(15,122,61,0.14);">
            ...
            <tr><td style="background:linear-gradient(135deg,#0b4d27 0%,#0F7A3D 55%,#34c46e 100%);padding:44px 32px;text-align:center;color:#ffffff;">
              <p style="margin:0;font-size:13px;letter-spacing:2px;text-transform:uppercase;opacity:.85;">DeepTalent &middot; Post-NYSC Corps Members</p>
              <h1 style="margin:12px 0 0 0;font-size:26px;font-weight:700;letter-spacing:-0.4px;">Verify your email</h1>
              <p style="margin:8px 0 0 0;font-size:14px;opacity:.9;">One quick step to activate your post corps member account.</p>
            </td></tr>
            <!-- Body -->
            <tr><td style="padding:40px 40px 8px 40px;">
              <p style="margin:0 0 14px 0;font-size:16px;font-weight:500;color:#111827;">Hi ${firstName},</p>
              <p style="margin:0 0 28px 0;font-size:15px;line-height:1.7;color:#4b5563;">Use the verification code below to confirm your email address and finish setting up your DeepTalent Post-NYSC account.</p>
              <!-- Code -->
              <table role="presentation" cellpadding="0" cellspacing="0" align="center" style="margin:0 auto 8px auto;"><tr>${digitCells}</tr></table>
              <p style="margin:20px 0 0 0;text-align:center;font-size:13px;color:#9ca3af;">This code expires in <strong style="color:#6b7280;">1 hour</strong>.</p>
            ...
            <tr><td style="padding:28px 40px 40px 40px;">
              <div style="border-top:1px solid #e5e7eb;padding-top:22px;text-align:center;font-size:13px;color:#9ca3af;">
                <p style="margin:0 0 6px 0;font-weight:600;color:#6b7280;">DeepTalent for Post-NYSC</p>
                <p style="margin:0;">© ${new Date().getFullYear()} DeepTalent. Cross-border talent infrastructure.</p>
              </div>
            </td></tr>
          </table>
    ...
    const result = await resend.emails.send({
      ...
      to: email,
      replyTo: REPLY_TO_EMAIL,
      subject: `${code} is your DeepTalent Post-NYSC verification code`,
      html: withReplyNotice(html),
    });
...
export async function sendNyscCertificateEmail(
  ...
  try {
    ...
    const html = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1.0"/><title>Your DeepTalent Post-NYSC certificate</title></head><body style="margin:0;padding:0;background-color:#eefbf3;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;color:#374151;">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#eefbf3;padding:32px 0;">
        <tr><td align="center">
          <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:18px;overflow:hidden;box-shadow:0 10px 30px rgba(15,122,61,0.14);">
            <!-- Header -->
            ...
            <tr><td style="padding:40px 40px 8px 40px;">
              <p style="margin:0 0 14px 0;font-size:16px;font-weight:500;color:#111827;">Hi ${firstName},</p>
              <p style="margin:0 0 20px 0;font-size:15px;line-height:1.7;color:#4b5563;">You've successfully completed the DeepTalent Global Workforce Readiness Programme for post-NYSC corps members. Your official certificate is attached to this email as a PDF, signed and issued by DeepTalent.</p>
    ...
    const result = await resend.emails.send({
      ...
      to: email,
      replyTo: REPLY_TO_EMAIL,
      subject: "Your DeepTalent Post-NYSC certificate of completion",
      html: withReplyNotice(html),
      attachments: [
        {
...
// ---------------------------------------------------------------------------
// Admin invite email — sent by super admin to invite a new admin
// ---------------------------------------------------------------------------
export async function sendAdminInviteEmail(opts: {
  ...
  try {
    const resend = getResendClient();
    const html = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1.0"/><title>You've been invited to DeepTalent Admin</title></head><body style="margin:0;padding:0;background-color:#f3f4f6;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;color:#374151;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#f3f4f6;padding:24px 0;"><tr><td align="center"><table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:14px;overflow:hidden;box-shadow:0 4px 12px rgba(15,23,42,0.06);"><tr><td style="background:linear-gradient(135deg,#0b1120 0%,#1a2744 100%);padding:48px 32px;text-align:center;color:#ffffff;"><p style="margin:0;font-size:13px;letter-spacing:1.2px;text-transform:uppercase;opacity:.7;color:#7b9ef8;">DeepTalent</p><h1 style="margin:10px 0 0 0;font-size:28px;font-weight:700;letter-spacing:-0.5px;">Admin account invitation</h1><p style="margin:8px 0 0 0;font-size:15px;opacity:.8;">You've been invited to manage the DeepTalent platform.</p></td></tr><tr><td style="padding:40px 32px 8px 32px;"><p style="margin:0 0 16px 0;font-size:16px;font-weight:500;color:#111827;">Hi ${opts.fullName},</p><p style="margin:0 0 18px 0;font-size:15px;line-height:1.7;color:#4b5563;"><strong>${opts.inviterName}</strong> has invited you to join DeepTalent as an administrator. Click the button below to set up your account — you'll be asked to create a password on the next screen.</p><div style="background:#fef3c7;border-left:4px solid #f59e0b;border-radius:6px;padding:14px 18px;margin:0 0 24px 0;font-size:13px;color:#92400e;line-height:1.7;"><strong>This link expires in 24 hours.</strong> If you don't complete setup in time, ask ${opts.inviterName} to send a new invite.</div><table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 0 28px 0;"><tr><td align="center" style="border-radius:8px;background:#3B5BDB;"><a href="${opts.setupLink}" style="display:inline-block;padding:13px 36px;font-size:15px;font-weight:600;color:#ffffff;text-decoration:none;border-radius:8px;">Set up my admin account</a></td></tr></table><p style="margin:0 0 8px 0;font-size:13px;color:#9ca3af;">Or copy this link:<br/><a href="${opts.setupLink}" style="color:#3B5BDB;word-break:break-all;">${opts.setupLink}</a></p></td></tr><tr><td style="padding:24px 32px;border-top:1px solid #e5e7eb;"><p style="margin:0;font-size:12px;color:#9ca3af;line-height:1.6;">If you did not expect this invitation, you can safely ignore this email. This link was generated by ${opts.inviterName} for ${opts.email} only.</p></td></tr></table></td></tr></table></body></html>`;

    const result = await resend.emails.send({
      from: FROM_EMAIL,
      to: opts.email,
      replyTo: REPLY_TO_EMAIL,
      subject: `You've been invited to manage DeepTalent`,
      html: withReplyNotice(html),
    });

    if (result.error) {
      console.error("[Resend] Admin invite email API error:", result.error);
      return { success: false, error: result.error.message || JSON.stringify(result.error) };
    }
    return { success: true, messageId: result.data?.id || "sent" };
  } catch (error: any) {
    console.error("[Resend] Admin invite email exception:", error?.message || error);
    return { success: false, error: error?.message || String(error) };
  }
}
